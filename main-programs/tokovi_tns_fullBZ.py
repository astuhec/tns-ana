...
dir1 = 'C:\\Users\\anast\\OneDrive\\Namizje\\1m\\poletje\\ta2nise5'
...
def kinetic(file = 'C:\\Users\\anast\\OneDrive\\Namizje\\1m\\poletje\\ta2nise5\\parameters\\parametri-kinetic.txt', extend=None, faktor=1.):
    seznam = []
...
def interaction(file = 'C:\\Users\\anast\\OneDrive\\Namizje\\1m\\poletje\\ta2nise5\\parameters\\parametri-interaction.txt'):
    seznam = []
    with open(file, 'r') as f:
        for line in f:
    ...
    for x, y, orb1, orb2 in interaction:
        ...
        for nu in nus:
            if u == l_idx:
                ...
                P[orb1-1, 0] = 1
                P[orb2-1, 1] = 1
                factor = np.exp(-1j * (Kxmesh * x * a + Kymesh * y * b) / 2).astype(np.complex64)
                U_kdelta = np.zeros((2, 2, Ny, nx), dtype=np.complex64)
                U_kdelta[0, 0] = factor
                U_kdelta[1, 1] = factor.conj()
                R = np.einsum('ij,jlxy->ilxy', P, U_kdelta)
